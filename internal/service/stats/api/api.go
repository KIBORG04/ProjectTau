...
func OnlineStatWeeksGET(c *gin.Context) {
	type Dates struct {
		DateFrom string `form:"dateFrom"`
		DateTo   string `form:"dateTo"`
	}
...
// ForecastHandler теперь отдает САМЫЙ ПОСЛЕДНИЙ недельный прогноз из истории
func ForecastHandler(c *gin.Context) {
	var latestForecast domain.ForecastHistory
	err := r.Database.Where("forecast_type = ?", "weekly").Order("created_at desc").First(&latestForecast).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			c.JSON(http.StatusOK, gin.H{})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not fetch weekly forecast"})
		return
	}
	c.Data(http.StatusOK, "application/json; charset=utf-8", latestForecast.Data)
}

// DailyForecastHandler теперь отдает САМЫЙ ПОСЛЕДНИЙ дневной прогноз из истории
func DailyForecastHandler(c *gin.Context) {
	var latestForecast domain.ForecastHistory
	err := r.Database.Where("forecast_type = ?", "daily").Order("created_at desc").First(&latestForecast).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			c.JSON(http.StatusOK, gin.H{})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not fetch daily forecast"})
		return
	}
	c.Data(http.StatusOK, "application/json; charset=utf-8", latestForecast.Data)
}
